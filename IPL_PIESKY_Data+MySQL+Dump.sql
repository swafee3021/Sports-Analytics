...
select * from ipl_match_schedule;
select * from ipl_stadium;
select count(match_id),s.stadium_id,s.stadium_name,s.city
from ipl_stadium s join ipl_match_schedule m
on s.stadium_id=m.stadium_id 
group by m.stadium_id,s.stadium_name,s.city,s.stadium_id
;
...
select * from ipl_bidding_details;
select * from ipl_team;
select t.team_id,t.team_name,count(bid_team)total_bid
from ipl_bidding_details d join ipl_team t
on d.bid_team=t.team_id
group by t.team_name;
-- 5.	Show the team id who won the match as per the win details.
select * from ipl_match;
select * from ipl_match_schedule;
select m.match_id,m.match_winner,m.win_details
from ipl_match m join ipl_match_schedule ms
on m.match_id = ms.match_id
group by m.match_winner,m.win_details;
-- 6.	Display total matches played, total matches won and total matches lost by team along with its team name.
select * from ipl_team;
select * from ipl_team_standings;
select t.team_name,sum(matches_played),sum(matches_won),sum(matches_lost)
...
select * from ipl_team;
select * from ipl_player;
select p.player_name Total_MI_Bowler
...
select count(player_role)total_all_rounder,t.team_id ,t.team_name
from ipl_team_players tp join ipl_team t 
on tp.team_id = t.team_id 
join ipl_player p
on tp.player_id = p.player_id  
where tp.player_role = 'all-rounder' 
group by t.team_name order by count(player_role) desc ;
